...
while (restartGame) {
    ...
    //Verifies the user's enter for a range number is a number greater than zero (NOTE: All numbers, positive or negative, have a default boolean value of true, except for zero which has a default boolean value of false. Also, NaN has a default boolean value of false.)
    ...
    //Prompts user to enter number of attempts allowed (AKA number of guesses)
    ...
    while (true){
        ...
        if (guess === randomNum) {
            alert(`CONGRATULATIONS! YOU GUESSED THE CORRECT NUMBER! WOOT!: ${randomNum}`);
            break;           

        //Check if user has any attempts left. If not, then the game ends and the number is displayed to the user.
        } else if (attempts === 0) {
